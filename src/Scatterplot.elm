...
view model =
    case model of
        ...
        Erfolg l ->
            let
                filteredCountrys =
                    filterAndReduceCountrys l.daten

                numberCountrys =
                    List.length l.daten

                numberFilterCountrys =
                    List.length filteredCountrys.data
...
update msg model =
    case msg of
        ErhalteText result ->
            case result of
                Ok fullText ->
                    ( Erfolg <| { daten = countryListe [ fullText ], xName = "Number of Conflicts", yName = "Fatalities"}, Cmd.none )
...
holenVonCsv x = 
    liste
        |> List.map
            (\datensatz ->
                Http.get
                    { url = "https://raw.githubusercontent.com/do46/IR-African_Conflicts-VAD/Viet-Anh-Do/Daten/" ++ datensatz
...
filterAndReduceCountrys my_countries =
    XyData "numberConflict" "fatalities" (List.filterMap country2point my_countries)
