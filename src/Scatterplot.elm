...
scatterplot model =
 -- funktionen und parameter deklarieren
    let
        ...
        kreisbeschriftung =
            Maybe.withDefault "Beschriftung nicht vorhanden"
                (Maybe.map .pointName (List.head model.data))
...
-- Linear scales are a good default choice for continuous
 -- quantitative data because they preserve proportional 
 -- differences. Each range value y can be expressed as a 
...
filterAndReduceCars my_cars =
    XyData "cityMPG" "retailPrice" (List.filterMap car2point my_cars)
-- List.filterMap filter out nothing value



pointLabel : String -> Int -> Int -> Point
pointLabel vehicleName cityMPG retailPrice =
...
car2point car =
    Maybe.map3 pointLabel (Just car.vehicleName) car.cityMPG car.retailPrice
-- input pretreatment
...
-- filteredCars is output of the function filterAndReduceCars w arg cars
        numberCars =
            List.length cars
...
-- get the length of the list
    ...
    Html.div []
        [ Html.p []
            [ Html.text "Originale Autoliste: "
            , Html.text <| String.fromInt numberCars
            , Html.text ", reduzierte Autoliste: "
